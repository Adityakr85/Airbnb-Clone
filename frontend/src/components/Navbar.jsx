...
export default function Navbar() {
  ...
  const formatWhenText = () => {
    if (activeTab === "flexible") {
      ...
      const months = flexibleMonths.map(id => new Date(...id.split("-").reverse()).toLocaleDateString("en-US", { month: "short" }));
    ...
    const isSameMonth = checkInDate.getMonth() === checkOutDate.getMonth() && checkInDate.getFullYear() === checkOutDate.getFullYear();
    
    let text = isSameDay ? start : isSameMonth ? `${start} – ${checkOutDate.getDate()}` : `${start} – ${fmt(checkOutDate)}`;
    return exactDatesFlex === "exact" ? text : `${text} ± ${exactDatesFlex} day${exactDatesFlex === "1" ? "" : "s"}`;
  };

  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white">
      <nav className={`relative flex items-center justify-between px-8 transition-all duration-500 ease-in-out ${(scrolled && !isExpanded) || hideSearchBar ? "h-20" : "h-28"}`}>
          ...
          <div className="absolute left-1/2 top-1/2 z-50 hidden -translate-x-1/2 -translate-y-1/2 md:block">
            {scrolled && !isExpanded ? (
              ...
              <div className="flex items-center gap-16 pt-4 transition-all duration-500">
                ...
                {TABS.map(tab => <TopTab key={tab.label} {...tab} />)}
              </div>
            )}
          </div>
        )}
        ...
        <div className="z-20 flex items-center gap-4">
          <Link to="/host" className="hidden rounded-full px-4 py-3 font-semibold transition hover:bg-gray-100 md:block">Become a host</Link>
      ...
      {!hideSearchBar && (
        <div className={`hidden justify-center transition-all duration-500 ease-in-out md:flex ${scrolled ? "absolute left-0 top-full z-40 w-full bg-white" : "w-full"} ${scrolled && !isExpanded ? "max-h-0 overflow-hidden pb-0 opacity-0" : "max-h-28 overflow-visible pb-8 opacity-100"}`}>
          <SearchBar
            ...
            setInfants={setInfants} pets={pets} setPets={setPets} formatGuestText={formatGuestText} formatWhenText={formatWhenText}
          />
        </div>
      )}
      
      {isExpanded && (<div onClick={() => { setIsExpanded(false); setOpenMenu(null);}} className="absolute left-0 top-full -z-10 h-[100vh] w-full bg-black/25 transition-opacity" />
      )}
    </header>
  );
}
...
function SmallSearchBar({ destination, when, guests, onMenuClick }) {
  ...
  const btnClass = "h-full max-w-40 truncate px-5 text-sm font-semibold transition hover:bg-gray-100";

  return (
    <div onClick={(e) => click(e, "where")} className="flex h-12 cursor-pointer items-center overflow-hidden rounded-full border border-gray-300 bg-white shadow-md transition-all duration-500 hover:shadow-lg">
      <button type="button" onClick={(e) => click(e, "where")} onMouseDown={stop} className={`${btnClass} rounded-l-full`}>{destination}</button>
      <div className="h-6 w-px bg-gray-300" />
      
      <button type="button" onClick={(e) => click(e, "when")} onMouseDown={stop} className={btnClass}>{when}</button>
      <div className="h-6 w-px bg-gray-300" />
      
      <button type="button" onClick={(e) => click(e, "guests")} onMouseDown={stop} className={`${btnClass} flex items-center rounded-r-full text-gray-500`}>
